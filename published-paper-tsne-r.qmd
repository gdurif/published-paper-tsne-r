...
::: {.callout-note}
Note from Computo editor: the following code define functions that will be used to run the various approaches, save/load the results and draw the result graphs. In order to save computational time, the code lauching the various approaches is not run during paper build. Results are rather saved into CSV files that are loaded to render the figures.
:::

```{r requirements}
## load requirements and source code
source(file.path("src", "requirements.R"))
source(file.path("src", "computing-setup.R"))
source(file.path("src", "saving-setup.R"))
source(file.path("src", "plot-setup.R"))
```

::: {.callout-note}
Note from Computo editor: we sub-samples 2000 images from the MNIST dataset instead of 6000 like in the original publication to save computing time.
:::
...
# MNIST dataset: https://www.openml.org/search?type=data&id=554
setOMLConfig(
    apikey = "c1994bdb7ecb3c6f3c8f3b35f4b47f1f",      # read-only API key
    cachedir = user_cache_dir(appname = "OpenML")     # user cache dir
)
mnist <- getOMLDataSet(data.id = 554L)

# extract sub-sample
set.seed(76987)
subsample <- sample.int(nrow(mnist$data), size = 2000)

# extract features and corresponding classes
X <- mnist$data %>% dplyr::slice(subsample) %>% dplyr::select(!class)
y <- mnist$data %>% dplyr::slice(subsample) %>% dplyr::select(class)
```

```{r computing-mnist}
#| eval: false
# run dimension reduction approaches
set.seed(1775486)
tsne_out <- run_tsne(X, n_comp = 2, perplexity = 40, class = y)
sammon_out <- run_sammon(X, n_comp = 2, class = y)
isomap_out <- run_isomap(X, n_comp = 2, k = 12, class = y)
lle_out <- run_lle(X, n_comp = 2, k = 12, class = y)

# save results
res_dir <- "results"
fs::dir_create(res_dir)
save_res(tsne_out, filename = file.path(res_dir, "mnist_tsne.csv"))
save_res(sammon_out, filename = file.path(res_dir, "mnist_sammon.csv"))
save_res(isomap_out, filename = file.path(res_dir, "mnist_isomap.csv"))
save_res(lle_out, filename = file.path(res_dir, "mnist_lle.csv"))
```

```{r fig-mnist-tsne-sammon}
...
tnse_out <- load_res(file.path(res_dir, "mnist_tsne.csv"))
draw_plot(tnse_out, title = "T-SNE", legend = TRUE) +
    scale_color_viridis_d(option = "turbo") +
    scale_shape_manual(values = rep(c(16,3,4), length.out = 10))

# Sammon mapping
sammon_out <- load_res(file.path(res_dir, "mnist_sammon.csv"))
draw_plot(sammon_out, title = "Sammon", legend = FALSE) +
    scale_color_viridis_d(option = "turbo") +
    scale_shape_manual(values = rep(c(16,3,4), length.out = 10))
```

```{r fig-mnist-isomap-lle}
...
# ISOMAP
isomap_out <- load_res(file.path(res_dir, "mnist_isomap.csv"))
draw_plot(isomap_out, title = "Isomap", legend = FALSE) +
    scale_color_viridis_d(option = "turbo") +
    scale_shape_manual(values = rep(c(16,3,4), length.out = 10))

# LLE
lle_out <- load_res(file.path(res_dir, "mnist_lle.csv"))
draw_plot(lle_out, title = "LLE", legend = FALSE) +
    scale_color_viridis_d(option = "turbo") +
    scale_shape_manual(values = rep(c(16,3,4), length.out = 10))
```

::: {.callout-note}
Note from Computo editor: the original Olivetti dataset contains 400 images of resolution 92 × 112 = 10,304 pixels. Here we use a modified version of the dataset with rescaled images of resolution 64 × 64 = 4,096 pixels.
:::

```{r olivetti-dataset}
## load and format Olivetti faces dataset
library(RnavGraphImageData)
data(faces)
X <- as.data.frame(t(faces))
colnames(X) <- paste0("px", 1:ncol(X))

# label data
# see https://github.com/jlmelville/snedata/blob/master/R/olivetti.R
n_people <- 40
n_pose <- 10

library(tidyr)
y <- expand_grid(people = 1:n_people, pose = 1:n_pose) %>% 
    mutate(class = people)
```

```{r computing-olivetti}
#| eval: false
# run dimension reduction approaches
set.seed(56776586)
tsne_out <- run_tsne(X, n_comp = 2, perplexity = 40, class = y)
...
res_dir <- "results"
fs::dir_create(res_dir)
save_res(tsne_out, filename = file.path(res_dir, "olivetti_tsne.csv"))
save_res(sammon_out, filename = file.path(res_dir, "olivetti_sammon.csv"))
save_res(isomap_out, filename = file.path(res_dir, "olivetti_isomap.csv"))
save_res(lle_out, filename = file.path(res_dir, "olivetti_lle.csv"))
```
...
# t-SNE manifold learning
tnse_out <- load_res(file.path(res_dir, "olivetti_tsne.csv"))
draw_plot(tnse_out, title = "T-SNE", legend = FALSE) +
    scale_color_manual(values = rep(colors, 10)) +
    scale_shape_manual(values = rep(markers, 4))

# Sammon mapping
sammon_out <- load_res(file.path(res_dir, "olivetti_sammon.csv"))
draw_plot(sammon_out, title = "Sammon", legend = FALSE) +
    scale_color_manual(values = rep(colors, 10)) +
    scale_shape_manual(values = rep(markers, 4))

# ISOMAP
isomap_out <- load_res(file.path(res_dir, "olivetti_isomap.csv"))
draw_plot(isomap_out, title = "Isomap", legend = FALSE) +
    scale_color_manual(values = rep(colors, 10)) +
    scale_shape_manual(values = rep(markers, 4))

# LLE
lle_out <- load_res(file.path(res_dir, "olivetti_lle.csv"))
draw_plot(lle_out, title = "LLE", legend = FALSE) +
    scale_color_manual(values = rep(colors, 10)) +
    scale_shape_manual(values = rep(markers, 4))
...
::: {.callout-note}
Note from Computo editor: Please note that (i) this dataset is too large for 
the code to be processed through continuous integration ; (ii) we did not 
reimplement this particular experiment (using random walk-based similarities). 
Only the result figure is joined.
:::
