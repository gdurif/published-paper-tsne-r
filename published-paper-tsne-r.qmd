...
digits <- getOMLDataSet(data.id = 554L)

# extract sub-sample
set.seed(76987)
subsample <- sample.int(nrow(digits$data), size = 2000)

# extract features and corresponding classes
X <- digits$data %>% dplyr::slice(subsample) %>% dplyr::select(!class)
y <- digits$data %>% dplyr::slice(subsample) %>% dplyr::select(class)
```
...
colnames(X) <- paste0("px", 1:ncol(olivetti))
